...
// ===== SISTEMA DE GARANTIA DE CARREGAMENTO DE IMAGENS =====
class ImageLoader {
  constructor() {
  ...
  init() {
    ...
    setInterval(() => this.processAllImages(), 30000)
  }

  processAllImages() {
    const images = document.querySelectorAll('img')
    images.forEach(img => this.ensureImageLoads(img))
  }

  ensureImageLoads(img, attempt = 1) {
    if (!img.src || this.loadedImages.has(img.src)) return
    ...
    const handleError = () => {
      ...
      if (attempt < this.retryAttempts) {
        setTimeout(() => {
          ...
          img.src = ''
          img.src = originalSrc + '?retry=' + attempt
          this.ensureImageLoads(img, attempt + 1)
        ...
        img.style.minHeight = img.closest('.item-image-placeholder') ? '180px' : 'auto'
      }
    }
    ...
    setTimeout(() => {
      if (!this.loadedImages.has(img.src) && !img.complete) {
        handleError()
      }
    }, 5000)
  }
...
function forceLoadMenuImages() {
  const menuImages = document.querySelectorAll('.menu-item img, .hero-logo, .thank-you-logo')
...
document.addEventListener('DOMContentLoaded', () => {
  new ImageLoader()
  setTimeout(forceLoadMenuImages, 1000)
  setInterval(forceLoadMenuImages, 10000)
})

document.addEventListener('visibilitychange', () => {
  if (!document.hidden) forceLoadMenuImages()
})
